param(
    ...
    # the bundle ships without it and speaker diarization is unavailable until the
    # .bin is added. Env override: MUESLI_SORTFORMER_BIN.
...
if ($pythonArch -eq "arm64") {
    ...
    if (-not [string]::IsNullOrWhiteSpace($sortformerBinSrc) -and (Test-Path $sortformerBinSrc)) {
        ...
        Write-Host "NOTE: no Sortformer NPU binary bundled (pass -SortformerBin or set MUESLI_SORTFORMER_BIN). Speaker diarization will be unavailable until the .bin is present."
    }
} else {
...
Optional add-ons (download extra dependencies into the bundled runtime):
  ...
  - Speaker diarization (NVIDIA Sortformer on the Hexagon NPU, Snapdragon/arm64 —
    pre-installed in the arm64 package):
      powershell -ExecutionPolicy Bypass -File .\setup-worker-runtime.ps1 -WithDiarizeSortformerNpu
